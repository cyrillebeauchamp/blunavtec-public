...
def create_poster(
    ...
    if coastline is not None and not coastline.empty:
        coastline_polys = coastline[coastline.geometry.type.isin(["LineString", "MultiLineString"])]
        if not coastline_polys.empty:
            try:
                coastline_polys = ox.projection.project_gdf(coastline_polys)
            except Exception:
                coastline_polys = coastline_polys.to_crs(g_proj.graph['crs'])
            coastline_polys.plot(ax=ax, color=THEME['coastline'], edgecolor='none', zorder=0.9)
